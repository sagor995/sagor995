# Sagor Ahamed

**Digital Innovation Technician · Unity & XR Developer · Manchester, UK**

I build immersive learning experiences — games that run inside 360° 
projection rooms, on VR headsets, and in students' browsers at the same 
time. Currently shipping XR curriculum tools at Wigan & Leigh College, 
where the work runs in actual lessons with actual students.

My background spans Unity, Unreal Engine, Flutter mobile apps, and web 
backends. I have an MSc in Games & Extended Reality from the University 
of Salford (2024). Before XR pulled me in completely, I spent three years 
in industry as a backend and mobile developer.

---

## What I'm Currently Building

**Quiz Quest Suite** — a series of five Unity XR educational games deployed 
in live FE curriculum. Runs on Igloo CAVE (360° projection), Meta Quest 3 
(MR passthrough), and phone browsers simultaneously. Students join via QR 
code — no app install. Sessions generate automatic PDF reports per player.

Five titles, all live:
- Quiz Quest CAVE — endless runner format, Igloo 360° projection
- Quiz Quest VR — Meta Quest headset, standalone
- Quiz Quest: Local Multiplayer — 4–6 controllers, real-time scoring, CAVE
- Quiz Quest MR Multiplayer — Meta Quest 3 passthrough, multiplayer
- Hospitality QUIZ — employability skills, Igloo CAVE, FE curriculum

→ [See the repo](https://github.com/sagor995/quiz-quest-multiplayer) · 
[Full writeup](https://sagorahamed.com/project.php?slug=quiz-quest-local-multiplayer-edition)

---
## Selected Projects

| Project | What it is | Stack | Status |
|---------|-----------|-------|--------|
| [Quiz Quest Suite](https://github.com/sagor995/quiz-quest-multiplayer) | 5 XR educational games, live in FE curriculum — CAVE, VR, MR, browser | Unity · Igloo · Meta Quest 3 · Firebase | ✅ Live |
| [Salford Campus Crisis](https://sagorahamed.com/project.php?slug=salford-campus-crisis-a-zombie-survival-game) | Third-person zombie survival game — MSc team project, University of Salford | Unity · C# · TPS · AI NPCs | ✅ Complete |
| [iBill Manager](https://sagorahamed.com/project.php?slug=ibillmanager) | ISP billing & operations app, offline-first architecture | Flutter · Dart · SQLite | ✅ [Google Play](https://play.google.com/store/apps/details?id=com.sagorahamed.ibill_manager) · [App Store](https://apps.apple.com/gb/app/ibillmanager-isp-billing/id6760230295) |

Full archive: [sagorahamed.com/projects.php](https://sagorahamed.com/projects.php)

---

## Tech Stack

**Game & XR**
`Unity (C#)` `Unreal Engine 5` `Igloo CAVE` `Meta Quest 3` `Igloo Toolkit` `OptiTrack` `Spout`

**Languages**
`C#` `C++` `JavaScript` `PHP` `Dart` `SQL` `Python`

**Mobile & Web**
`Flutter` `Firebase` `Node.js` `Tailwind CSS` `MySQL`

**Tools**
`Git / GitHub` `Unity New Input System` `QuestPDF` `Agile`

---

## Background

```
2025 – Now   Digital Innovation Technician · Wigan & Leigh College
2024 – 2025  Technical Demonstrator, XR Lab · University of Salford
2023 – 2024  MSc Games & Extended Reality · University of Salford (Distinction)
2022 – 2023  Backend Developer · IT Zone, Dhaka
2020 – 2022  Mobile Developer · Digital WebFace Ltd, Dhaka
2015 – 2020  BSc Computer Science & Engineering · AUST, Dhaka
```

---

## Certifications

- Unity for Industry — Unity Technologies (2026)
- How to Talk to Robots AI Bootcamp (2025)
- Unity Junior Programmer — Unity Technologies / Credly (2024)
- C# for Unity Game Development — Coursera / University of Colorado (2022)
- CS50 Introduction to Computer Science — Harvard (2020)
- Google IT Automation with Python — Coursera / Google (2020)

---

## Get In Touch

I'm open to Unity/XR developer roles, particularly anything involving 
immersive learning, CAVE systems, or multiplayer XR. Visa sponsorship 
required for roles outside the UK.

[sagorahamed.com](https://sagorahamed.com) · 
[linkedin.com/in/sagor59](https://linkedin.com/in/sagor59) · 
sagor12ahamed@gmail.com
